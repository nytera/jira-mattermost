...
| Module | Role |
|--------|------|
| `service/` | Orchestration package. The only place that coordinates Mattermost + Jira + repository. Read this first. `IncidentBotService` is being split into per-domain **mixin** files assembled in `service/coordinator.py` (`class IncidentBotService(PostmortemMixin, ThreadSummaryMixin, …)`); re-exported from `service/__init__.py` so `from mm_jira_bot.service import IncidentBotService` is unchanged. Done so far: `_thread_summary.py` (ThreadSummaryMixin), `_postmortem.py` (PostmortemMixin — `generate_incident_postmortem` + Jira field plumbing `_set_time_to_fix`/`_apply_postmortem_validity`/`_ensure_postmortem_jira_issue` + `_postmortem_thread_context`). `_shared.py` holds cross-domain runtime primitives (`ActionResult`, summary texts, `_PROMPT_KEY_*`) as the import-graph leaf that breaks coordinator↔mixin cycles. Mixin typing convention: state-attrs only as `__init__` declares them (`mattermost`/`jira`/`llm` → `Any`), cross-domain calls as inline `if TYPE_CHECKING:` stubs, `log = get_logger("mm_jira_bot.service")` in every file. |
