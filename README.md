...
_Карточка ручного инцидента (инцидентный канал, на корневой пост не от бота):_

| Контрол | Права | Результат |
|---|---|---|
| **➕ Создать задачу** | да | Создаёт Jira issue (без alert-полей), карточка сменяется на контролы ниже |
