...
По умолчанию бот реагирует на реакции и нажатия кнопок от любого пользователя. Чтобы он учитывал действия только определённых людей, перечислите их **логины** (через запятую, можно с `@`) в `MATTERMOST_AUTHORIZED_USERNAMES` — бот сам резолвит их в user-id на старте. Пусто = разрешено всем.

- Под ограничение попадают: реакции (`Инцидент`, `Ложный`/`Ожидаемый`, чекмарк-постмортем), кнопки/меню валидности и инцидента, slash-команда `/incident`.
- **Обратная связь доступна всем** — кнопка и форма фидбэка не ограничиваются.
- Если логин не нашёлся (опечатка), он логируется (`authorized_users.unresolved`), остальные продолжают работать. Если резолв упал целиком (Mattermost недоступен), гейт остаётся открытым (fail-open) — реальный барьер обеспечивает сетевая изоляция эндпоинта.
...
`INTERACTIVE_BUTTONS_ENABLED=false` принудительно включает режим «только эмодзи»: бот не добавляет ни одну интерактивную карточку (карточка алерта, карточка ручного инцидента, обратная связь) даже при заданном `SERVICE_PUBLIC_URL`, и единственной точкой входа остаются эмодзи-реакции. По умолчанию `true`.
...
| Инцидентный | галочка на **корне** треда | да | `END`-поле + (если есть LLM) постмортем; для ручного треда — создаёт Jira-задачу |
| Инцидентный | галочка на **ответе** | — | Игнор |
| Инцидентный | `incident` / Ложный / Ожидаемый | — | Игнор («не в алерт-канале») |
| Любой другой | любой | — | Игнор |
| Любой | нерелевантный эмодзи (👍 и т.п.) | — | Игнор |

**Кнопки/меню в треде** (появляются при `SERVICE_PUBLIC_URL` и `INTERACTIVE_BUTTONS_ENABLED` ≠ `false`; иначе режим «только эмодзи»):
...
| **📝 Summary** | да | Тред → LLM → краткое саммари ответом в тред (без `LLM_API_TOKEN` — эфемерный no-op) |
...
| **🏁 Завершить** | да | Полный постмортем + end-time (как галочка; нужен `LLM_API_TOKEN`); после клика кнопка меняется на **✅ Завершено** |
| **📝 Саммари** | да | Лёгкое саммари в тред, Jira не трогает |
...
- Если задан `MATTERMOST_DUTY_MENTION` (например `:look: @sre-ads-duty`), бот
  ...
  (упоминание во вложении не нотифицирует).
- По клику **➕ Создать задачу** создаётся Jira issue (без alert-only полей), а
  ...
  **📝 Саммари**.
- **🏁 Завершить** = тот же полный постмортем, что и галочка (PM-шаблон
  с участниками/автором/ссылкой, отчёт комментарием в Jira, название через LLM,
  end-time). Выбранную валидность **не перетирает**: если стоит `Ложный`, он таким
  и останется.
- **📝 Саммари** — лёгкое summary треда в тред, Jira не трогает (это не полный
  отчёт из постмортема).
