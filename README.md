...
- публикует в incident-тред фактологическое саммари-инцидентный отчёт (отдельный LLM-промпт, не из текста постмортема, запись в Jira не делает). Плейсхолдер при этом показывает поэтапный статус («⏳ Шаг 1/3: генерирую постмортем…» → «Шаг 2/3: отправляю постмортем в Jira…» → «Шаг 3/3: генерирую саммари…»), а текст саммари при `LLM_STREAM=true` дописывается в тред по мере генерации (LLM-стрим), затем заменяется финальным отчётом;
