...
6. Бот публикует сообщение в `MATTERMOST_INCIDENT_CHANNEL_ID`, обновляет Jira `Valid Incident = Валидный` и добавляет комментарий со ссылкой на incident-сообщение. После подтверждения бот также отвечает в тред исходного алерта о том, что инцидент заведён (ссылка на Jira, валидность, ссылка на сообщение в канале инцидентов). Имя подтвердившего показывается как `Имя Фамилия (@username)`, а не как сырой `user_id`.
...
- **🚨 Инцидент** → полное подтверждение инцидента (`:incident:`-флоу: пост в канал инцидентов, комментарий);
...
|---|---|---|---|
| Алертный | `incident` | да | Полное подтверждение инцидента (пост в инцидентный канал, `Валидность=Валидный`, коммент, ответ в треде) |
