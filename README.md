...
**Кнопки/меню в треде** (появляются при `SERVICE_PUBLIC_URL`):
...
| **🚨 Инцидент** | да | Полное подтверждение инцидента (как реакция `incident`) |
...
| **🏁 Завершение инцидента** | да | Полный постмортем + end-time (как галочка; нужен `LLM_API_TOKEN`) |
...
- По клику **➕ Создать задачу** создаётся Jira issue (без alert-only полей), а
  ...
  `Валидный` → пишет в поле, как в алерте), **🏁 Завершение инцидента** и
  **📝 Саммари**.
- **🏁 Завершение инцидента** = тот же полный постмортем, что и галочка (PM-шаблон
...
в инцидентном канале **ту же карточку контролов** — меню валидности, 🏁 Завершение
инцидента и 📝 Саммари — но **без** «Создать задачу», т.к. Jira-задача уже создана.
Карточка постится ответом под сообщением инцидента сразу при подтверждении.
