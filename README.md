...
Под первым сообщением о создании в треде firing-алерта и ручного инцидента бот публикует **памятку дежурному SRE** — боксированный список доступных эмодзи-реакций (без подсказок по кнопкам). Отключается `DUTY_HELP_ENABLED=false` (по умолчанию `true`). В тред инцидента, заведённого из алерта, памятка не дублируется — она уже была в треде алерта.
...
**Памятка дежурному** (`DUTY_HELP_ENABLED` ≠ `false`, по умолчанию вкл.): под первым сообщением о создании в треде firing-алерта и ручного инцидента бот постит боксированный список эмодзи-реакций. В тред инцидента из алерта не дублируется.
