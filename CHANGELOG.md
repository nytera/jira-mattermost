...
- **Пост об инциденте — три бокса вместо двух.** Сверху отдельный бокс с названием
  алерта; в среднем боксе первая строка — статус `**Новый инцидент**` / `**Закрытый
  инцидент**` со ссылкой на Jira-задачу (отдельный пункт «Задача Jira» убран); пункт
  «Подтвердил» переименован в «Автор», «Время подтверждения» — во «Время алерта».
  Третий бокс — пересланный алерт, как раньше. Green-flip при закрытии теперь меняет
  статус-текст в среднем боксе (и красит все боксы), а не кружок в заголовке.
- **Тестовые каналы в shadow-режиме — живая песочница.** Настроенные
  `MATTERMOST_TEST_ALERT_CHANNEL_ID` / `MATTERMOST_TEST_INCIDENT_CHANNEL_ID` теперь
  пишутся **вживую** (реальные посты/треды/реакции), а не зеркалятся в аудит-канал.
  Алерт из тестового канала подтверждается инцидентом **в тестовом инцидентном
  канале**, и там же его можно закрыть — с green-flip заголовка на месте. Jira при
  этом остаётся застабанной (`ADS-TEST`), реальные задачи не создаются. Прод и
  реальные каналы — без изменений; тестовые каналы активны только под
  `READ_ONLY_MODE`.
- **Саммари треда:** под ответом ЛЛМки теперь второй блок-аттачмент со статусом
  отправки в Jira — зелёный (добавлено комментарием), серый (задача не найдена) или
  красный (ошибка Jira).
- **Шпаргалка дежурного в инциденте** больше не пишет «завершить инцидент +
  постмортем»: при закрытии постмортем не генерируется (нарратив — только по кнопке
  саммари), текст исправлен на «завершить инцидент».
