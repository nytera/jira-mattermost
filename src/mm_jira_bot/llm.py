...
технично и по существу, без воды и канцелярита, корректной инженерной
терминологией. Заполняй все разделы шаблона; будь подробным там, где это
подкреплено тредом, и кратким там, где данных мало — не теряй деталей, которые
...
Все времена указывай по московскому времени в формате HH:MM. Не добавляй
преамбулу, code fences и служебные пояснения, верни только отчёт строго по
