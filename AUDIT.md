...
(`debug_admin._validity_status`, запросы «empty validity») сперва смотрят на `valid_incident`
...
**Суть.** `TicketStatsCollector.collect()` синхронно зовёт `repository.debug_summary()`
...
оффлоадит тот же `debug_summary` через `asyncio.to_thread` (`web.py:91`), а путь метрик — нет.
...
рендерить gauge'и из значения, сэмплированного off-loop (кэш `debug_summary` с обновлением
...
**Где:** `debug_admin.py:27-43`. Источник `debug-1`. conf=medium.
...
`debug_admin_save_setting` (пустое = сброс/удаление), либо в `_prompt_settings_payload`
использовать `if db_value:` под truthiness резолвера.

## C10 (low) — `recreate`: check-then-act гонка может породить дубли Jira-issue
**Где:** `service/_debug.py:157-207`. Источник `debug-2`. conf=low.

**Суть.** `debug_recreate_jira_issue` читает `ticket.jira_issue_key` (`:157/165`), и при
отсутствии ключа (или `force=true`) зовёт `_create_jira_issue` (`:181`) до персиста через
`replace_jira_issue` (`:202`). Между чтением и созданием нет ни лока, ни in-flight маркера.
...
**Оговорка.** Автообновление панели само recreate **не** зовёт (только перечитывает список) —
...
query-строки debug-панели (`debug_admin.py:662-672`) и эхо-возвращается в ответе (`:677`) —
...
## Отклонённая находка
- **`repository-3`** (`repository.py:261-266`) — «`debug_summary.pending_jira` расходится с
